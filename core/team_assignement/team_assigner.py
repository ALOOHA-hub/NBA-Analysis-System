...
from constants import (
    TEAM_ASSIGNER_MODEL_NAME,
    TEAM_ASSIGNER_TEAM_1_LABEL,
    TEAM_ASSIGNER_TEAM_2_LABEL,
    TEAM_ASSIGNER_RESET_FREQUENCY
)
...
class TeamAssigner:
    ...
    """
    def __init__(self,
                 team_1_label=TEAM_ASSIGNER_TEAM_1_LABEL,
                 team_2_label=TEAM_ASSIGNER_TEAM_2_LABEL,
                 model_path=TEAM_ASSIGNER_MODEL_NAME
                 ):
        """
        ...
        self.team_1_label = team_1_label
        self.team_2_label = team_2_label
    ...
    def get_player_color(self, frame, bbox):
        ...
        # Ensure image is valid (not empty)
        if image.size == 0:
            return self.team_2_label

        # Convert to PIL Image
        rgb_image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        pil_image = Image.fromarray(rgb_image)

        classes = [self.team_1_label, self.team_2_label]
    ...
    def get_player_team(self, frame, player_bbox, player_id):
        ...
        team_id = 2
        if player_color == self.team_1_label:
            team_id = 1

        self.player_team_dict[player_id] = team_id
        return team_id

    def get_player_teams_across_frames(self, video_frames, player_tracks, stub_path=None):
        """
        ...
        player_assignment = self.stub_manager.load(stub_path)
        ...
        for frame_num, player_track in enumerate(player_tracks):        
            ...
            if frame_num % TEAM_ASSIGNER_RESET_FREQUENCY == 0:
                self.player_team_dict = {}
        ...
        if stub_path:
            self.stub_manager.save(player_assignment, stub_path)
